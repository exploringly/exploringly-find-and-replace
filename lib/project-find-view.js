...
class ProjectFindView {
          ...
          $.span({className: 'header-item options-label pull-right'},
            ...
            $.span({className: 'btn-group btn-toggle btn-group-options'},
              $.button({ref: 'regexOptionButton', className: 'btn option-regex'},
                $.svg({className: "icon", innerHTML: `<use xlink:href="#exploringly-find-and-replace-icon-regex" />`})
              ),
              $.button({ref: 'caseOptionButton', className: 'btn option-case-sensitive'},
                $.svg({className: "icon", innerHTML: `<use xlink:href="#exploringly-find-and-replace-icon-case" />`})
              ),
              $.button({ref: 'wholeWordOptionButton', className: 'btn option-whole-word'},
                $.svg({className: "icon", innerHTML:`<use xlink:href="#exploringly-find-and-replace-icon-word" />`})
              )
            )
          )
        ),
  ...
  didShow() {
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      atom.tooltips.add(this.refs.findAllButton, {
        title: "Find All",
        keyBindingCommand: 'exploringly-find-and-replace:search',
        keyBindingTarget: this.findEditor.element
      })
    );
  }
  ...
  handleEvents() {
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'exploringly-find-and-replace:use-selection-as-find-pattern': () => this.setSelectionAsFindPattern()
    }));

    this.subscriptions.add(atom.commands.add(this.element, {
      'exploringly-find-and-replace:focus-next': () => this.focusNextElement(1),
      'exploringly-find-and-replace:focus-previous': () => this.focusNextElement(-1),
    ...
    let afterSearch = () => {
      if (atom.config.get('exploringly-find-and-replace.closeFindPanelAfterSearch')) {
  ...
  showResultPane() {
    ...
    let openDirection = atom.config.get('exploringly-find-and-replace.projectSearchResultsPaneSplitDirection');
