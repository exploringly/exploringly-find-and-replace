...
class FindView {
  ...
  render() {
    return (
      $.div({tabIndex: -1, className: 'exploringly-find-and-replace'},
          ...
          $.span({className: 'header-item options-label pull-right'},
            ...
            $.span({className: 'btn-group btn-toggle btn-group-options'},
              $.button({ref: 'regexOptionButton', className: 'btn'},
                $.svg({className: "icon", innerHTML: '<use href="#exploringly-find-and-replace-icon-regex" />'})
              ),

              $.button({ref: 'caseOptionButton', className: 'btn'},
                $.svg({className: "icon", innerHTML: '<use href="#exploringly-find-and-replace-icon-case" />'})
              ),

              $.button({ref: 'selectionOptionButton', className: 'btn option-selection'},
                $.svg({className: "icon", innerHTML: '<use href="#exploringly-find-and-replace-icon-selection" />'})
              ),

              $.button({ref: 'wholeWordOptionButton', className: 'btn option-whole-word'},
                $.svg({className: "icon", innerHTML: '<use href="#exploringly-find-and-replace-icon-word" />'})
              )
            )
          )
        ),
        ...
        $.section({className: 'input-block replace-container'},
          ...
          $.div({className: 'input-block-item'},
            ...
            $.div({className: 'btn-group btn-group-replace-and-find'},
              $.button({ref: 'replaceNextAndFindButton', className: 'btn btn-next-and-find'}, 'Replace And Find')
            ),
        ...
        $.svg({style: {display: 'none'}, innerHTML: `
          <symbol id="exploringly-find-and-replace-icon-regex" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
          ...
          <symbol id="exploringly-find-and-replace-icon-case" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
          ...
          <symbol id="exploringly-find-and-replace-icon-selection" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
          ...
          <symbol id="exploringly-find-and-replace-icon-word" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
          ...
          <symbol id="exploringly-find-and-replace-context-lines-before" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
            <rect opacity="0.6" x="2" y="11" width="16" height="2"></rect>
            <rect x="2" y="7" width="10" height="2"></rect>
            <rect x="2" y="3" width="10" height="2"></rect>
          </symbol>

          <symbol id="exploringly-find-and-replace-context-lines-after" viewBox="0 0 20 16" stroke="none" fill-rule="evenodd">
  ...
  didShow() {
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      atom.tooltips.add(this.refs.regexOptionButton, {
        title: "Use Regex",
        keyBindingCommand: 'exploringly-find-and-replace:toggle-regex-option',
        keyBindingTarget: this.findEditor.element
      }),
      atom.tooltips.add(this.refs.caseOptionButton, {
        title: "Match Case",
        keyBindingCommand: 'exploringly-find-and-replace:toggle-case-option',
        keyBindingTarget: this.findEditor.element
      }),
      atom.tooltips.add(this.refs.selectionOptionButton, {
        title: "Only In Selection",
        keyBindingCommand: 'exploringly-find-and-replace:toggle-selection-option',
        keyBindingTarget: this.findEditor.element
      }),
      atom.tooltips.add(this.refs.wholeWordOptionButton, {
        title: "Whole Word",
        keyBindingCommand: 'exploringly-find-and-replace:toggle-whole-word-option',
        keyBindingTarget: this.findEditor.element
      }),

      atom.tooltips.add(this.refs.nextButton, {
        title: "Find Next",
        keyBindingCommand: 'exploringly-find-and-replace:find-next',
        keyBindingTarget: this.findEditor.element
      }),
      atom.tooltips.add(this.refs.findAllButton, {
        title: "Find All",
        keyBindingCommand: 'exploringly-find-and-replace:find-all',
        keyBindingTarget: this.findEditor.element
      })
    );
  }
  ...
  handleEvents() {
    ...
    this.subscriptions.add(atom.commands.add(this.findEditor.element, {
      ...
      'exploringly-find-and-replace:confirm': () => this.confirm(),
      'exploringly-find-and-replace:show-previous': () => this.showPrevious(),
      'exploringly-find-and-replace:find-all': () => this.findAll()
    }));
    ...
    this.subscriptions.add(atom.commands.add(this.element, {
      ...
      'exploringly-find-and-replace:focus-next': this.toggleFocus.bind(this),
      'exploringly-find-and-replace:focus-previous': this.toggleFocus.bind(this),
      'exploringly-find-and-replace:toggle-regex-option': this.toggleRegexOption.bind(this),
      'exploringly-find-and-replace:toggle-case-option': this.toggleCaseOption.bind(this),
      'exploringly-find-and-replace:toggle-selection-option': this.toggleSelectionOption.bind(this),
      'exploringly-find-and-replace:toggle-whole-word-option': this.toggleWholeWordOption.bind(this)
    }));
  ...
  handleFindEvents() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'exploringly-find-and-replace:find-next': () => this.findNext({focusEditorAfter: true}),
      'exploringly-find-and-replace:find-previous': () => this.findPrevious({focusEditorAfter: true}),
      'exploringly-find-and-replace:find-all': () => this.findAll({focusEditorAfter: true}),
      'exploringly-find-and-replace:find-next-selected': this.findNextSelected.bind(this),
      'exploringly-find-and-replace:find-previous-selected': this.findPreviousSelected.bind(this),
      'exploringly-find-and-replace:use-selection-as-find-pattern': this.setSelectionAsFindPattern.bind(this)
    }));
  }

  handleReplaceEvents() {
    this.refs.replaceNextButton.addEventListener('click', this.replaceNext.bind(this));
    this.refs.replaceNextAndFindButton.addEventListener('click', this.replaceNextAndFind.bind(this));
    this.refs.replaceAllButton.addEventListener('click', this.replaceAll.bind(this));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'exploringly-find-and-replace:replace-previous': this.replacePrevious.bind(this),
      'exploringly-find-and-replace:replace-next': this.replaceNext.bind(this),
      'exploringly-find-and-replace:replace-next-and-find': this.replaceNextAndFind.bind(this),
      'exploringly-find-and-replace:replace-all': this.replaceAll.bind(this),
    }));
  }
  ...
  confirm() {
    this.findNext({focusEditorAfter: atom.config.get('exploringly-find-and-replace.focusEditorAfterSearch')});
  }

  showPrevious() {
    this.findPrevious({focusEditorAfter: atom.config.get('exploringly-find-and-replace.focusEditorAfterSearch')});
  }

  liveSearch() {
    let findPattern = this.findEditor.getText();
    if (findPattern.length === 0 || (findPattern.length >= atom.config.get('exploringly-find-and-replace.liveSearchMinimumCharacters') && !this.model.patternMatchesEmptyString(findPattern))) {
  ...
  replaceNextAndFind() {
    this.replaceAndFind('findNext', 'firstMarkerIndexStartingFromCursor');
  }
  ...
  // This code was copied from "replace" above
  replaceAndFind(nextOrPreviousFn, nextIndexFn) {
    this.search();
    this.findHistoryCycler.store();
    this.replaceHistoryCycler.store();
  ...
  markersUpdated(markers) {
    ...
    if (this.model.getFindOptions().findPattern) {
      ...
      if (this.findEditor.getElement().hasFocus() && results > 0 && atom.config.get('exploringly-find-and-replace.scrollToResultOnLiveSearch')) {
  ...
  updateReplaceEnablement() {
    ...
    if (canReplace) {
      ...
      this.refs.replaceNextAndFindButton.classList.remove('disabled');

      this.replaceTooltipSubscriptions.add(atom.tooltips.add(this.refs.replaceNextButton, {
        title: "Replace Next",
        keyBindingCommand: 'exploringly-find-and-replace:replace-next',
        keyBindingTarget: this.replaceEditor.element
      }));
      this.replaceTooltipSubscriptions.add(atom.tooltips.add(this.refs.replaceNextAndFindButton, {
        title: "Replace Next And Find",
        keyBindingCommand: 'exploringly-find-and-replace:replace-next-and-find',
        keyBindingTarget: this.replaceEditor.element
      }));
      this.replaceTooltipSubscriptions.add(atom.tooltips.add(this.refs.replaceAllButton, {
        title: "Replace All",
        keyBindingCommand: 'exploringly-find-and-replace:replace-all',
        keyBindingTarget: this.replaceEditor.element
      }));
    } else {
      this.refs.replaceAllButton.classList.add('disabled');
      this.refs.replaceNextButton.classList.add('disabled');
      this.refs.replaceNextAndFindButton.classList.add('disabled');

      this.replaceTooltipSubscriptions.add(atom.tooltips.add(this.refs.replaceNextButton, {
        title: "Replace Next [when there are results]"
      }));
      this.replaceTooltipSubscriptions.add(atom.tooltips.add(this.refs.replaceNextAndFindButton, {
        title: "Replace Next And Find [when there are results]"
      }));
      this.replaceTooltipSubscriptions.add(atom.tooltips.add(this.refs.replaceAllButton, {
  ...
  showWrapIcon(icon) {
    if (!atom.config.get('exploringly-find-and-replace.showSearchWrapIcon')) return;
    let editor = this.model.getEditor();
    if (!editor) return;
