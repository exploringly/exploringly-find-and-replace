...
module.exports = class ResultsModel {
  ...
  search (findPattern, pathsPattern, replacePattern, options) {
    ...
    const leadingContextLineCount = atom.config.get('exploringly-find-and-replace.searchContextLineCountBefore')
    const trailingContextLineCount = atom.config.get('exploringly-find-and-replace.searchContextLineCountAfter')
  ...
  setResult (filePath, result) {
    if (result) {
      this.addResult(filePath, result)
    } else {
      this.removeResult(filePath)
    }
  }
  ...
  addResult (filePath, result) {
    ...
    this.emitter.emit('did-add-result', {filePath, result, filePathInsertedIndex, filePathUpdatedIndex})
  }
  ...
  removeResult (filePath) {
    if (this.results[filePath]) {
      ...
      this.emitter.emit('did-remove-result', {filePath, filePathRemovedIndex})
    }
  }
