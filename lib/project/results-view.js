...
class ResultsView {
  constructor({model}) {
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      atom.commands.add(this.element, {
        ...
        'exploringly-find-and-replace:copy-path': this.copyPath.bind(this)
      })
    );
  }
        ...
        if (result) {
          ...
          return atom.workspace
            .open(result.filePath, {
              pending,
              split: reverseDirections[atom.config.get('exploringly-find-and-replace.projectSearchResultsPaneSplitDirection')]
            })
            .then(editor => {
  ...
  toggleLeadingContextLines() {
    if (this.model.getFindOptions().leadingContextLineCount > 0) {
      ...
      const searchContextLineCountBefore = atom.config.get('exploringly-find-and-replace.searchContextLineCountBefore');
  ...
  toggleTrailingContextLines() {
    if (this.model.getFindOptions().trailingContextLineCount > 0) {
      ...
      const searchContextLineCountAfter = atom.config.get('exploringly-find-and-replace.searchContextLineCountAfter');
